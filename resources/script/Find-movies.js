		...
		//Shows
			...
			//Check if show exists
				...
				if (newShow == true) {
					...
					allShows[showIndex].sSeasons = []; //Initialise the object array
					//if doesn't exist then save in xml but as a temporary movie (gets deleted once the real one is queried and saved)
				...
				//tableRow("summaryTable2", "push episode", "show", allShows[showIndex].sTitle, "season", thisSeason);
				
				//add season to list of seasons if not already in list
				if (allShows[showIndex].sSeasons.indexOf(thisSeason) < 0) {
					//tableRow("summaryTable2", "push season", thisShow, thisSeason);
					allShows[showIndex].sSeasons.push(thisSeason);
				}
			}
		}
	}
...
function cleanFileName(orig, filePath) {
	...
	if (thisSeason !== null) { //If it is then extract the show name season/episode
		//tableRow("summaryTable2","cleanFileName", newName, filePath);
		
		//Check if the file name is in the format "[show name] s#e#", if not
		if (!(/.+?(?=s\d{1,2}e\d{1,2})/gi.test(newName))){
			//tableRow("summaryTable2","cleanFileName", newName, "guess show: " + filePath.match(/^\w:\\([^\\]+\\)*([^\\]+)\\.*$/i)[2]);
			//prepend the folder name (it is likely the name of the show)
			var parentFolder = filePath.match(/^\w:\\([^\\]+\\)*([^\\]+)\\.*$/i);
			parentFolder = (typeof parentFolder[2] !== "undefined")?parentFolder[2]:parentFolder[1];
			newName = parentFolder + newName; //in the format "[show name] s#e#"
		}
